...
void DAQDevice::SetDataReceivedCallback(DAQCommon::CallbackFunc callback)
{
    g_callback = callback;
}
...
void DAQDevice::MeasurementStateChangedEvent(MeasurementStatus status, std::string message)
{
    ...
    {
        g_callback(status, message.c_str());
    }
}
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if((ret += root_files[channel].open_file(out_file_path, DAQ_meas_settings.event_samples)) < 0)
                        {
                                ...
                                snprintf(resp_buff, 256, "CMD_OPEN_NEW_ROOT_FILE ERROR code:-4 (open file) %s\n", out_file_path);
                        }
                        else
                        {
